...
@pytest.mark.mobile
def test_home_club_filter_matches_mongo(
    ...
    clubs_page = home.get_content().open_clubs()
    clubs_page.club_filter.select_city(city_name)

    actual_cards = sorted(clubs_page.club_filter.get_all_club_cards(), key=lambda item: item.key)
