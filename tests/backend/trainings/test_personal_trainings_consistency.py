...
UPDATED_AT_MONTHS = 2  # месяцев назад
UPDATED_AT_DAYS   = 0  # дней назад

CREATED_AT_YEARS  = 0  # лет назад
CREATED_AT_MONTHS = 0  # месяцев назад
CREATED_AT_DAYS   = 0  # дней назад
# =========================================

_COL_HEADER = f"{'№':<4} {'USP ID':<26} {'User ID':<26} {'Init':<5} {'Count':<6} {'Tickets':<8} {'Hist':<5} {'Status':<7} {'Updated At':<22} {'Created At':<22}"
_COL_SEP = "=" * 190
...
def _compute_date_from(years: int, months: int, days: int) -> datetime:
    ...
    return datetime(target_year, target_month, target_day,
                    now.hour, now.minute, now.second) - timedelta(days=days)
...
def _results_to_html_table(results_list: list) -> str:
    ...
    for r in results_list:
        cells = [
            ...
            r["status"], _fmt_date(r["updated_at"]), _fmt_date(r["created_at"]),
        ]
        trs.append("<tr>" + "".join(f"<td>{c}</td>" for c in cells) + "</tr>")
    table = f'<table border="1" cellpadding="4" cellspacing="0"><thead><tr>{th_cells}</tr></thead><tbody>{"".join(trs)}</tbody></table>'
...
def _attach_results(name: str, results_list: list):
    ...
    allure.attach(_results_to_text_table(results_list), name=name, attachment_type=allure.attachment_type.TEXT)
    allure.attach(_results_to_html_table(results_list), name=f"{name} — таблица", attachment_type=allure.attachment_type.HTML)
...
def test_personal_trainings_count_consistency(db):
    ...
    with allure.step("Получение записей userserviceproducts"):
        ...
        if SPECIFIC_USP_ID:
            record = usp_col.find_one({"_id": ObjectId(SPECIFIC_USP_ID), "isDeleted": False, "type": "SPECIALIST"})
            ...
            if use_updated:
                updated_from = _compute_date_from(UPDATED_AT_YEARS, UPDATED_AT_MONTHS, UPDATED_AT_DAYS)
                query["updated_at"] = {"$gte": updated_from}
            if use_created:
                created_from = _compute_date_from(CREATED_AT_YEARS, CREATED_AT_MONTHS, CREATED_AT_DAYS)
                query["created_at"] = {"$gte": created_from}

            sort_field = "updated_at" if use_updated else "created_at" if use_created else "_id"
    ...
    with allure.step("Получение связанных данных из БД (batch запросы)"):
        ...
        tickets_counts = {
            ...
            for doc in tt_col.aggregate([
                {"$match": {"userServiceProduct": {"$in": usp_ids}, "isUsed": False, "status": "active", "isDeleted": False}},
        ...
        history_counts = {
            ...
            for doc in hist_col.aggregate([
                ...
                {"$sort": {"created_at": -1}},
                {"$group": {"_id": "$userServiceProduct", "lastRecord": {"$first": "$$ROOT"}}},
            ])
        }
    ...
    if SPECIFIC_USP_ID:
        ...
        if UPDATED_AT_YEARS or UPDATED_AT_MONTHS or UPDATED_AT_DAYS:
            parts.append(f"updated_at: {UPDATED_AT_YEARS}г {UPDATED_AT_MONTHS}м {UPDATED_AT_DAYS}д назад")
        if CREATED_AT_YEARS or CREATED_AT_MONTHS or CREATED_AT_DAYS:
            parts.append(f"created_at: {CREATED_AT_YEARS}г {CREATED_AT_MONTHS}м {CREATED_AT_DAYS}д назад")
        period_label = ", ".join(parts) if parts else "без фильтра по дате"

    allure.attach(
        f"Окружение: {DB_ENVIRONMENT.upper()}\nПериод: {period_label}\nВсего: {len(results)}  OK: {ok_count}  FAIL: {failed_count}",
    ...
    if failed_count > 0:
        _attach_results(f"Записи с расхождениями ({failed_count})", failed_records)

        usp_tt_mismatch = [r for r in failed_records if r["count"] != "N/A" and r["tickets_count"] != r["count"]]
        if usp_tt_mismatch:
            _attach_results(f"Расхождения USP ↔ TrainingTickets ({len(usp_tt_mismatch)})", usp_tt_mismatch)
    else:
