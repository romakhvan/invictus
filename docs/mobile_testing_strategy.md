...
Драйвер и пакет приложения общие; различается только точка входа (выбор режима) и последующие экраны.

## Правило онбординга и usermetadatas

- Если у клиента уже есть запись в коллекции `usermetadatas`, после авторизации онбординг в приложении не показывается.
- Для mobile-сценариев, где после OTP-авторизации ожидается сразу главная или состояние `NEW_USER`, нужно использовать пользователя `role: potential`, у которого есть запись в `usermetadatas`.
- Для сценариев, которые проверяют именно полный флоу онбординга, нужно использовать новый номер или пользователя без записи в `usermetadatas`.
