...
## Типы backend-сценариев

Сейчас backend-набор делится на три типа сценариев:

| Marker | Тип | Назначение | Где запускается по умолчанию |
|--------|-----|------------|-------------------------------|
| `backend_check` | Gate-check | Проверка бизнес-правил и инвариантов, может валить основной прогон | `tests_to_run_backend.txt` |
| `backend_monitoring` | Monitoring | Операционный мониторинг и отчёты с Allure-вложениями | `tests_to_run_backend_monitoring.txt` |
| `backend_research` | Research | Разовые аналитические и исследовательские сценарии | В дефолтные профили не входит |

Marker-ы назначаются централизованно в `tests/backend/conftest.py`, пока каталог `tests/backend/` ещё не разрезан на отдельные папки `checks/`, `monitoring/` и `research/`.
Backend CLI-опции и MongoDB-фикстуры тоже живут в `tests/backend/conftest.py`; корневой `tests/conftest.py` больше не содержит backend-специфику.

**Правило:** новые аналитические или репортинговые сценарии нельзя добавлять в основной backend suite. Если сценарий не является gate-проверкой бизнес-правила, он не должен попадать в `tests_to_run_backend.txt`.
...
Основной gate-прогон backend checks запускается через `run_tests.py` и список `tests_to_run_backend.txt`:

```bash
python run_tests.py   # MODE = "backend" в run_tests.py
```

Документация по Allure для этого потока собрана в `docs/allure/README.md`. Канонические правила оформления backend-отчётов находятся в `docs/allure/backend_reporting_rules.md`.

Отдельный monitoring-профиль запускается тем же раннером, но с отдельным списком:

```bash
# в run_tests.py:
# MODE = "backend"
# FILE = "tests_to_run_backend_monitoring.txt"
python run_tests.py
```
...
```bash
pytest tests/backend -m backend_check -v
pytest tests/backend -m backend_monitoring -v
pytest tests/backend -m backend_research -v
...
**Правило**: каждый новый backend-файл сначала классифицируется как `check`, `monitoring` или `research`, и только потом попадает в соответствующий профиль запуска. Добавлять аналитические сценарии в `tests_to_run_backend.txt` нельзя.
...
- **`test_webkassa_monitoring.py`** — за последние 7 дней проверяет статус фискальных чеков по клубам: `success / error / отсутствует`. Исключает тестовые клубы. Вывод: клубы с проблемами + примеры ошибочных транзакций. Идёт через отдельный monitoring-профиль.
- **`test_recent_transactions.py`** — анализирует транзакции текущего дня с 14:55 UTC+5, группирует по `instalmentType`, выводит примеры `fail`. Это monitoring/reporting-сценарий, а не gate-check.
- **`test_bonus_usage_distribution.py`** — показывает распределение успешных транзакций по доле бонусов и строит аналитические таблицы без падения основного suite. Идёт через monitoring-профиль.
...
| `test_statistics_2025.py` | PROD | Исследовательская статистика: первые визиты, стрики, потери клиентов, персональные тренировки |
| `test_rabbitholev2_no_duplicate_users.py` | PROD | Дубликаты подписок за 14 дней |
| `test_high_frequency_clients_no_subscription.py` | PostgreSQL | Исследовательский сценарий: клиенты с высокой частотой посещений без подписки |
...
- **Перед добавлением файла в запуск определить тип сценария** — `backend_check`, `backend_monitoring` или `backend_research`.
- **Не добавлять monitoring/research в gate-suite** — основной `tests_to_run_backend.txt` должен содержать только проверки, которые действительно должны валить прогон.
- **Один backend entrypoint** — `db`, `prod_db`, `backend_env` и `period_days` поддерживаются только через `tests/backend/conftest.py`.
